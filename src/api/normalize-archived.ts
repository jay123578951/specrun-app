/**
 * archived 清單／詳情的原始輸出 → App 型別（純函式，web 與 M4 Tauri 版共用）。
 *
 * 與 normalize-parked 的分工相同：route 只做 IO，所有解析集中在這裡。
 * 資料源同樣是檔案層直讀（design D1），所以日期、進度、順序全都是這裡算出來的
...
/** archive 的目錄命名慣例 `YYYY-MM-DD-<name>`（design D3） */
...
/**
 * 目錄名拆成日期與名稱兩欄，卡片才不用把日期在名稱裡再讀一次（design D3）。
...
/**
 * 清單排序（spec 清單排序）：日期新→舊、同日名稱序、無日期墊底。
 ...
 * ——tabs 是 route 現場列舉的（design D4），每個 tab 恰好一個檔案。唯讀由呼叫端保證
 * （archived store 沒有寫入面，design D7）。
 */
