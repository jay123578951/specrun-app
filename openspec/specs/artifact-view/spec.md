...
artifact 內容 SHALL 以 Markdown 渲染，支援 GFM 表格與 task list；task list 的 checkbox SHALL 呈現勾選狀態但 MUST NOT 可互動（勾選為 C4 範圍）；code block SHALL 有語法高亮。整個詳情檢視 SHALL 為唯讀，MUST NOT 提供任何編輯能力。

#### Scenario: tasks 的唯讀 checkbox
- **WHEN** tasks.md 含已勾與未勾的 task list 項目
...
### Requirement: 載入與新鮮度
每次進入詳情或切換 change，系統 SHALL 重新取得該 change 的詳情資料，MUST NOT 以快取取代重取。有快取的 change SHALL 立即顯示上次取得的內容、不等待請求完成；重取完成後內容有差異 SHALL 靜默更新並保留捲動位置，無差異 MUST NOT 重繪。詳情快取 SHALL 跨 session 持久化，持久層不可用時 SHALL 靜默降級為記憶體快取。清單載入完成後，系統 SHALL 依序背景預載各 active change 的詳情；使用者點開尚未預載的 change 時，該項 SHALL 優先取得。此預載屬掛載時載入一次的延伸；除此、進入／切換與手動刷新外，詳情 MUST NOT 自動重新載入（watcher 為 C3 範圍）。無快取可顯示時（罕見的墊底路徑），內容面板 SHALL 空著或僅顯示小型載入提示，MUST NOT 顯示 skeleton 動畫，MUST NOT 顯示前一個 change 的內容。
