...
桌面形態的設定讀寫、CLI 執行檔解析、change 與 spec 的讀取（清單與詳情），以及檔案操作面（tasks 勾選寫入、park／unpark、parked 與 archived 的清單與詳情）SHALL 由外殼自持，MUST NOT 經由本地 API server。尚未移轉的資料路徑在過渡期間 MAY 仍經本地 API server；移轉完成前，桌面開發通路 SHALL 併跑它。
