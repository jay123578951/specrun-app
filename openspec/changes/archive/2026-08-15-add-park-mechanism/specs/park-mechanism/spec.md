...
目標專案無正常 `.git/` 目錄時（非 git repo，或 `.git` 為檔案的 git worktree），park 動作 SHALL 為禁用狀態並附說明提示；MUST NOT 靜默失敗或改存他處。
...
### Requirement: 操作失敗呈現
park／unpark 因檔案系統錯誤失敗時 SHALL 顯示自動消失的 toast 並保留現狀，MUST NOT 留下半完成狀態可見於清單（清單以重新列舉後的實際目錄狀態為準）。
