...
| T4c | add-tauri-gateway-opener | T4b 的後半：開啟檔案所在位置、artifact 內容裡的外部連結，兩者共用同一個外殼通道（Tauri 官方 opener plugin）。桌面形態下兩者皆已可用：Settings 的兩顆開啟位置按鈕可按、按下後 Finder 選取該項（**「目前專案」那一列的選取行為已由 `open-project-folder-directly` 推翻，改為直接開啟資料夾本身；當時記下的已知限制「專案放在桌面第一層時 Finder 只開到桌面、什麼都沒選取」隨之解除，成因是選取，而該列已不再走選取**）；artifact 內容中的外部連結點擊後交給系統預設瀏覽器開啟，不論可勾選、唯讀、pending 哪一種檢視模式。開啟失敗一律以非阻斷提示告知，不再是靜默的「點了沒事」 | ✅ 2026-09-18 archived |
| T5 | add-app-release | 名稱、bundle id、版本定版、`tauri build`、README 安裝說明＋首發 Release（logo／icon 已移至 T1.5） |⏭️ 下一個 |
