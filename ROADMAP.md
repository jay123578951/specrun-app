...
| T4c | add-tauri-gateway-opener | T4b 的後半：開啟檔案所在位置、artifact 內容裡的外部連結，兩者共用同一個外殼通道（Tauri 官方 opener plugin）。桌面形態下兩者皆已可用：Settings 的兩顆開啟位置按鈕可按、按下後 Finder 選取該項；artifact 內容中的外部連結點擊後交給系統預設瀏覽器開啟，不論可勾選、唯讀、pending 哪一種檢視模式。開啟失敗一律以非阻斷提示告知，不再是靜默的「點了沒事」 | ✅ 2026-09-18 archived |
