import type { ResolveDeps, VerifyResult } from './cli-resolver'
import { describe, expect, it, vi } from 'vitest'
import { CLI_COMMAND, pickCommandPath, pickVersion, resolveOnPath, resolveWith } from './cli-resolver'

/**
 * 只測解析的決策層（降級順序、覆寫優先、輸出取值）——spawn 全部注入。
 * 真正的 login shell 與 `--version` 由實機驗收（tasks 10.3），比照 folder-picker 的分法。
 */
