...
2. **專案清單**：全展開直接點擊切換（不用下拉）；過多才收合（門檻細節 propose 時定，暫定 5–6 個之後折疊「Show all」）；每項掛進行中 change 數徽章；hover 浮現 ✕ 移除（跳確認，文案講明只移出清單不動磁碟）；底部「＋ Add project」。
...
| 拖曳 park/unpark | hover 按鈕已夠快，拖曳邊界情況不成比例 |
